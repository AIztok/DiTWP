...
Die Basis bei der Bauwerksplanung ist meistens die Geometrie. 
Die Bauwerke werden im bestehenden Raum / Gelände gebaut, somit ist bereits die vorzufindende Geometrie wichtig (neben der Geologie, Eigentumsrechten usw.), da es ggf. die Bauwerksform, neben den Vorgaben der Nutzung, eingrenzt.
Ebenso bestimmt z.B. das Gelände die Baugrubengröße, zusammen mit der Geologie auch die erforderlichen Verbaumaßnahmen.

In weiterer Folge wird dann die vorgesehene (Neubau) oder bereits vorhandene (Bestand) Bauwerksform mittels 2D oder 3D geometrischen Modellen abgebildet. 
...
Die BREPs brauchen nicht wie in diesem Fall rechteckig zu sein, sondern können beliebigen Formen / Funktionen folgen.

![[211_3D_Modell.png]]

Im folgenden sehen wir uns ein paar übliche Datenformate zum speichern und bearbeiten von geometrischen Modellen.
## DWG und DXF
...
Wichtige Informationen sind aber oft in Form von Text:
- Textliche und / oder Numerische Angaben am Plan
- Hinweise am Plan
- dazugehörige Dokumente
oder schematisch:
- Linienform mit Legende
- Farben (z.B. Rot Neubau, Gelb Rückbau, Schwarz Bestand, Magenta Kontext)

Beispiel:
![[211_VO_Planauschnitt_Farben.png|300]]

![[211_VO_Planauschnitt_Legende.png|500]]

Visualisierung einer DXF Datei des Rahmens aus der Übung über den Autodesk Dienst:
https://autode.sk/3XyXi7l

Bild aus dem Viewer:
![[211_VO_Autodesk_Viewer_visual.png]]

Dxf Datei ist auch [hier abgelegt](https://github.com/AIztok/DiTWP_Data/blob/main/211_VO/DiTWP_GH_DXF_Export.dxf).
...
Wir sehen, dass die Formatierung der Datei ähnlich ist wie das IFC Format [[134_IFC]], da es auch die [EXPRESS data modeling language/sprache](https://en.wikipedia.org/wiki/EXPRESS_(data_modeling_language)) verwendet.
...
### Beispiel: Geometrie (IFC Datei) ins Speckle und Rhino/Grasshopper übertragen

Wir zeigen das Beispiel anhand unseres Modells von der Übung, wo wir die Geometrie der Bodenplatte auf Speckle hochladen, dort analysieren und in Rhine/Grasshopper einlesen und uns die geometrischen Daten ansehen.

Dafür ist es zuerst notwendig auf Speckle ein Workspace und Project zu erstellen, siehe hierzu:

[[312_UE#Neues Workspace einrichten]]

![[312_UE#Upload IFC]]


![[112_UE#Einlesen der Geometrie aus Speckle]]
...
Zum mitmachen bei der Vorlesung sind die Dateien [hier abgelegt](https://github.com/AIztok/DiTWP_Data/tree/main/211_VO/SOFISTIK).
Drinnen befindet sich:
- .dat Datei (Ascii Format) - Eingabe File für die App `Teddy` bzw. kann auch über `SSD` gelesen werden
- .cdb Datei - die Datenbasis 
- .plb Datei - Ausgabe Bericht der Berechnung
...
## Beispiel: Sofistik Datei Lesen

Wir nehmen das Beispiel des Halbrahmens von hier:

![[121_VO#Beispiel IFC Structural und SAF in einer FE Software (Sofistik) exportieren]]

Und schauen uns, wie in den vorherigen Kapiteln dargestellt, die Daten die bei einem FE-Modell (das Beispiel oben ist bereits "berechnet" und alles in der Datenbasis - .cdb Datei - abgelegt).
...
Wir schauen uns das BIM Modell des Halbrahmens, das wir über Speckle (siehe [[321_Speckle]]) aus Grasshopper in eine BIM Software ([[015_Blender & BonsaiBIM]]) übertragen haben (siehe [[412_UE#Übertragen des Modells von Speckle ins BlenderBIM]]).

## Nur Geometrie

Wir schauen uns an ein IFC File wo nur die Geometrie abgebildet wird.
Wenn die einzelnen Bauteile (Decke oder Wand) ausgewählt werden, ist zu sehen, dass keine zusätzlichen Informationen im Modell sind:

<iframe title="Speckle" src="https://app.speckle.systems/projects/eba1405a21/models/fc7b88fbec#embed=%7B%22isEnabled%22%3Atrue%7D" width="600" height="400" frameborder="0"></iframe>



IFC Datei liegt hier:
https://github.com/AIztok/DiTWP_Data/blob/main/211_VO/GEO/DiTWP_Halbrahmen_Geometrie_v00.ifc

## PSET, Qto

Im folgenden Fenster, wenn ein Bauteil (Decke oder Wand) selektiert wird, können die einzelnen Psets (Property Sets) oder Quantities angesehen werden.

![[211_VO_PSET_Speckle.png]]
...
IFC Datei liegt hier:
https://github.com/AIztok/DiTWP_Data/blob/main/211_VO/PSET/DiTWP_Halbrahmen_PSET_v00.ifc

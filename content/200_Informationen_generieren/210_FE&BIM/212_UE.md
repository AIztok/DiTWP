...
Fit Line funktioniert jedoch nicht mehr, wenn die Breite größer wird als die Länge, weil dann die Schwerpunktlinie quer verlaufen wird. Hier kann alternativ auch der untere Weg gewählt werden.

### Mittels Ermittlung der Schwerpunkte der Stirnflächen

Aus der Liste der BREPs (Decke und Wand) wählen wir eine aus:
- List Item Komponente mit Index 0 oder 1

Zerlegen das BREP in Einzelteile:
- Deconstruct BREP

![[212_GH_brep2line_centroid_1.png]]

Wählen die Flächen / Stirnseiten von wo bis wo wir die Schwerpunktlinie ziehen wollen (List Item). Da es bei den Cubus 6 Flächen gibt, wurde ein Number Slider von 0 bis 5 erstellt.

![[212_GH_brep2line_centroid_2.png]]

Z.B. es wurden die grün markierten Flächen gewählt:

![[212_GH_brep2line_centroid_3.png]]

Danach werden für die zwei gewählten Flächen = Stirnflächen, die Schwerpunkte ermittelt:
- Area Komponenten, Centroid ist der Schwerpunkt
und diese mit einer Linie verbunden:
- Line Komponente.

![[212_GH_brep2line_centroid_4.png]]
...
## Export in Excel mit TT-Toolbox
...
## Export Textdatei aus Grasshopper

Alternativ werden die Querschnittswerte als eine Textdatei aus Grasshopper ausgespielt. Folgende Komponenten werden verwendet:

- `CONCATENATE` baut aus mehreren Teilen einen Text (String) zusammen
- `PANEL` der Text wird in Panels geschrieben

![[212_GH_textdatei_01.png]]

Rechtklick auf das rechte Panel und auswählen 

![[212_GH_textdatei_02.png]]

Grundsätzlich wird in den Ordner wo die .gh Datei liegt gespeichert, es muss ein Name angegeben werden und ein Dateityp (.txt, .dat, ...):

![[212_GH_textdatei_03.png]]

Danach wird das `Stream contents` aktiviert - mit jeder Änderung in der .gh Datei wird die Datei automatisch neu gespeichert / überschrieben:

![[212_GH_textdatei_04.png]]

Der Inhalt der Datei ist folgender:

![[212_GH_textdatei_05.png]]
## Export in Excel mit Excel+ 

Ein sehr elegantes Tool zum speichern in Excel ist Excel+ (siehe [[012_Rhino_3D#Excel+]]).

![[212_UE_excle_plus.png]]


## Export in Excel mit Speckle

Daten können auch mittels Speckle aus dem Modell ins Excel exportiert werden.

Dafür wird im Projekt aus Speckle ein neues Modell erstellt:

![[212_UE_Speckle_new_model.png]]


![[212_UE_Speckle_create.png]]

Nachdem das Modell erstellt ist, kann der Link kopiert werden:

![[212_UE_Speckle_link.png]]


In Grasshopper können dann die Daten auf Speckle versendet werden:

![[212_UE_Speckle_gh.png]]

Um die Daten dann in Excel zu importieren muss Speckle Add-In in Excel installiert sein (siehe [[321_Speckle#Excel Connector]]):

![[212_Import_excel_speckle.gif]]
...
Der erstellte Code in der .dat Datei (gleicher Ordner wie die Grasshopper Datei):
...
### Nutzlast
Die Nutzlast wird als eine Punktlast mit 50 kN, die am Punkt in der Mitte der Linie (Decke) wirkt wie folgend definiert, wir beziehen uns dabei mit der Komponente:
`Point on line` 
auf die Mitte (0,5 * Länge) der Linie:
...
Die Lastfallnummern werden in den Komponente LC Attributes eingelesen.
Hier wird auch definiert:
- Einwirkung 
- Faktor für das Eigengewicht
- Bezeichnung

Die Lastfallnummern werden dabei aus den oben angegebenen Lastfallnummern in eine Liste zusammengeführt. In gleicher Reihenfolge sollten die Einwirkungsarten und Faktoren fürs Eigengewicht definiert werden, damit diese richtig zugewiesen werden können:
...
Wir nehmen die bereits erstellt Vorlage (.sofistix):

[.sofistix Vorlage für SSD](https://aiztok.github.io/SBB/docs/FH_SBB_SSD-Vorlage_V01.sofistix) 

Somit haben wir bereits einen Taskbaum in unserer Sofistik Datei vordefiniert:

![[422_UE_Sofistik_Taskbaum.png]]


Wir stützen uns dabei auf die LVA Stahlbetonbau und den EInführungsbeispiel, wo alles noch näher erläutert ist:

https://aiztok.github.io/SBB/011_Einfuehrungsbeispiel.html

## Materialdefinition

Die Materialdefinition erfolgt wie folgend:
...
Wir nutzen die Funktion zum einlesen einer Excel Datei bzw. der einzelnen Zellen:

```javascript
+PROG TEMPLATE
HEAD Einlesen Excel
LET#XLS.NAME "DiTWP_212-UE_Punkte.xlsx"  $ define the XLSX file
LET#XLS.SHEET "QS"    $ define the Sheet name

STO#B XLS.READ(1,1)      $ read data from cell (row = 1, column = 5)
STO#Hdeck XLS.READ(1,2)      $ read data from cell (row = 1, column = 5)
STO#Hwand XLS.READ(1,3)      $ read data from cell (row = 1, column = 5)
...
<text>
Deckenbreite #(B,10.2), Decken-QS-höhe #(Hdeck,10.2)
Wandbreite #(B,10.2), Wand-QS-höhe #(Hwand,10.2)
</text>
...
srec 1 h #Hwand[m] b #B[m] mno 1 mrf 101 titl 'Wand'
srec 2 h #Hdeck[m] b #B[m] mno 1 mrf 101 titl 'Decke'

end
```

Alternativ kann statt der Excel Datei die Textdatei eingelesen werden:

```javascript
+PROG TEMPLATE
HEAD Textdatei

#include Querschnitte.txt
...
Im wenn im gleichen Ordner die Grasshopper Datei und das Sofistik File liegen, dann kann einfach mit folgenden Code die ganze `sofiload.dat` in SSD eingelesen werden:

```javascript
#include sofimshc.dat 
```

## Definition der Einwirkungen

Diese Definition in  `SOFILOAD` dient der Benennung der Einwirkungen:
```javascript
...
Im wenn im gleichen Ordner die Grasshopper Datei und das Sofistik File liegen, dann kann einfach mit folgenden Code die ganze `ase.dat` in SSD eingelesen werden:
...
In Sofistik Structural Desktop (SSD)
Die Überlagerung erfolgt wie folgend:

### Kombinationsvorschriften
Doppelclick auf die Kombinationsvorschriften, hier werden bereits automatisch vom Programm die folgenden Kombinationen erstellt:

![[212_UE_Sofi_kombinationsvorschriften.png]]

### Überlagerungen
Rechtclick auf den leeren Raum in der Projektnavigation und Task einfügen wählen:

![[212_UE_Sofi_task_einfuegen.png]]

Auswählen "Überlagerungen für Kombinationsvorschriften":

![[212_UE_Sofi_ueberlagerung.png]]

Das Programm erstellt für alle definierten Kombinationsvorschriften die Überlagerungen. Diese können, wenn gewünscht, angepasst werden, gelöscht, oder neue hinzugefügt werden. Mit Bestätigung OK, werden die Überlagerungen berechnet:

![[212_UE_Sofi_ueberlagerung_2.png]]


Für die textliche Eingabe (Programmcode) der Überlagerung siehe hier: 

[SBB/1.Übung/Überlagerung](https://aiztok.github.io/SBB/011_Einfuehrungsbeispiel.html#%C3%BCberlagerung-der-ergebnisse)

## Bemessung

Für die textliche Eingabe (Programmcode) der Bemessung siehe hier: 

[SBB/1.Übung/Bemessung](https://aiztok.github.io/SBB/011_Einfuehrungsbeispiel.html#bemessung-der-stabelemente)

### Tasks Bemessung Stäbe

Tasks einfügen:
- Bemessung GZT (Stäbe)
- Bemessung GZG (Stäbe)

![[212_UE_Sofi_Bemessung_1.png]]

### GZT Bemessung

Der Task Bemessung GZT (Stab) wird mit Doppelclick geöffnet.

Im Reiter "Allgemein" werden die Speicherlastfälle für GZT angezeigt - das Programm findet die automatisch entspr. der Überlagerung.

![[212_UE_Sofstik_GZT_1.png]]

Im Reiter "Textausgabe" kann der Umfang des Ausgabe kontrolliert werden:

![[212_UE_Sofstik_GZT_2.png]]

Im Reiter "Grafische Ausgabe" kann die graphische Ausgabe angepasst werden:

![[212_UE_Sofstik_GZT_3.png]]

Mit Rechtclick auf den Task und "Ergebnisse" können die textlicher und grafischen Ergebnisse angesehen werden:

![[212_UE_Sofstik_GZT_4.png]]

Report browser:

![[212_UE_Sofstik_GZT_5.png]]
### GZG Bemessung

Im gleichen Sinne wird auch die GZG Bemessung durchgeführt.
Es muss vorher die GZT Bemessung durchgeführt gewesen sein, weil z.B. für die Ermittlung der Rissweite, die aus GZT ermittelte Bewehrungsverteilung (Speicherlastfall 1) aus Ausgangsgröße gewählt wird. Die Bemessungsergebnisse GZG werden in den Speicherlastfall 2 gespeichert:

![[212_UE_Sofistik_GZG_1.png]]

## Ergebnisse

### Graphische Ausgabe

Um die weiteren Ergebnisse visuell zu zeigen, kann das Programm `Graphic` bzw. der Task `Interaktive Grafik` verwendet werden.

Mehr dazu hier:
[SBB/Grafische_Darstellung_der_Ergebnisse](https://aiztok.github.io/SBB/023_Q&A_zur_Hausuebung.html)

### Tabellarische Ausgabe

Für die tabellarische Ausgabe kann der Task `Interaktive Listen und Grafiken` verwendet werden:

![[212_UE_Sofstik_Tabelle_1.png]]

Task Doppelclick und auswählen "Neue Tabelle erstellen":

![[212_UE_Sofstik_Tabelle_2.png]]

Auswählen welches Ergebnis soll angezeigt werden, z.B. die Längsbewehrung Rang 1 (untere Bewehrung):

![[212_UE_Sofstik_Tabelle_3.png]]

Und es wird die Tabelle angezeigt:

![[212_UE_Sofstik_Tabelle_4.png]]

Diese Tabelle kann entweder im `Report Browser` gedruckt werden (dann unter Tabelle/Ausgabe in PLB) oder als Excel Tabelle exportiert werden:
- Tabelle
- Ausgabe nach Excel
- Einstellungen
- Auswählen den Pfad und Dateinamen (sinnvoll in gleichen Ordner wie die Sofistik Datei), Auswählen Worksheetnamen und bestätigen OK

![[212_UE_Sofstik_Tabelle_5.png]]

Danach muss noch der Task berechnet werden (Rechtklick und Berechne):

![[212_UE_Sofstik_Tabelle_6.png]]

Und das Excel File wird erstellt:

<iframe 
height = 400
width = 100%
src="https://docs.google.com/spreadsheets/d/e/2PACX-1vRy3M4pd_3A3xomOgeWdPoa2cfTfXrnvSRZ8TpAgBgIDqmRHf-c9_4jCB-TgchtKw/pubhtml?gid=1188912618&amp;single=true&amp;widget=true&amp;headers=false">
</iframe>
...
## Excel Datei mit den Punkten und Querschnittabmessungen
...
## Grasshopper File mit Excel+ und Speckle export nach Excel

Die ergänzende GH Datei mit export nach Excel mittels Excel+ und Speckle export kann [hier hertuntergeladen](https://github.com/AIztok/DiTWP_Data/blob/main/212_UE/DiTWP_W24_UE_1_Excel-Plus.gh) werden.
